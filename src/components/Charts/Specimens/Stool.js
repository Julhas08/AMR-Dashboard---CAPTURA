...
class Stool extends Component {
	...
	render() {
		const stool = this.props.stool.map((response) => {
			return response.Code;
		});
		// Sort spcimen by desc
   		var sortedSpecimen = this.props.stool.sort(({ ['Number of isolates']: a }, {['Number of isolates']: b }) => b-a);
		
		// Generate color key 
	    var chartObj = [];
	    for (var i = 0; i < 5; i++) {
	      if (typeof sortedSpecimen[i] != 'undefined') { 
	      	chartObj.push({name: sortedSpecimen[i].Code, y: sortedSpecimen[i]['Number of isolates'], color: this.props.colors[5]})
	    	}
	    }
